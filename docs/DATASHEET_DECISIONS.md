...
- Выбрать точный MPN стабилитрона ограничения `VGS` Q1 и сверить его напряжение с
  абсолютным максимумом `VGS = +/-25 В` DMP4015SSS.
- Выбрать точные низкопадные MPN диодов D3-D6 и проверить худший случай
  `V5_BUFFER >= 4,5 В`; без этого диапазон питания AHCT формально не закрыт.
